...
import { getInternshipApplicationsApi } from "../../../api/admin/admin";
import TableLoader from "../../../components/loader/TableLoader";
import { formatDate } from "../../../utils/helper";
import Pagination from "../../../components/Pagination";

export interface InternshipApplicationFilterState {
  page: number;
  query?: string;
  location?: string;
  status?: string;
...
const InternshipApplicationList: React.FC = () => {
  const [filters, setFilters] = useState<InternshipApplicationFilterState>({
    page: 0,
    query: "",
    location: "",
    status: "",
  ...
  const debouncedTextInputs = useMemo(
    () =>
      debounce((updatedFields: Partial<InternshipApplicationFilterState>) => {
  ...
  const handleFilterChange = (
    key: keyof InternshipApplicationFilterState,
    value: string
  ) => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await getInternshipApplicationsApi(debouncedFilters);
            ...
            {isLoading ? (
              ...
              applicationsData?.map((item, i: number) => (
                <tr key={i} className="even:bg-gray-100">
                  ...
                  <td className="px-4 py-2">{item.internship?.title || "-"}</td>
                  <td className="px-4 py-2">
                    {item.internship?.minStipend} -{" "}
                    {item.internship?.maxStipend}
                  </td>
                  <td className="px-4 py-2">
