...
interface UpdateImgCatModalProps {
  isOpen: boolean;
  onClose: () => void;
  category: Category | null;
}

const UpdateImgCatModal: React.FC<UpdateImgCatModalProps> = ({
  isOpen,
  onClose,
  category,
}) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, name }: { id: number; name: string }) =>
      ImageCatApi.updateImgCategory(id, name),
    onSuccess: () => {
    ...
    onError: () => {
      ToastMessage.error("Failed to update category.");
    },
    onSettled: () => {
      setProcessing(false);
    },
...
export default UpdateImgCatModal;
